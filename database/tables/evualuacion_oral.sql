...
CREATE TABLE evaluacion_oral (
    ...
    CONSTRAINT evlauacion_oral_ibfk_1 FOREIGN KEY (id_cohorte) -- Clave foránea que relaciona la evaluación con su cohorte
    REFERENCES cohortes (id_cohorte) ON DELETE CASCADE,
    CONSTRAINT evlauacion_oral_ibfk_2 FOREIGN KEY (created_by) -- Clave foránea que relaciona la evaluación con su creador
    REFERENCES usuarios (id_usuario),
    CONSTRAINT evlauacion_oral_ibfk_3 FOREIGN KEY (updated_by) -- Clave foránea que relaciona la evaluación con su último actualizador
    REFERENCES usuarios (id_usuario)
) ENGINE=InnoDB;
